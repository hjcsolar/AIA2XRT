...
netfile='/Volumes/hdata3/Be_thin_Open/cnn/aia2Be_Thin64x20L2log1.mod' #模型文件名
xrtchannel='Be_thin_Open'


# netfile='/Volumes/hdata3/Open_Al_mesh/cnn/aia2Al_mesh_64x20L2log1.mod'
...
file='../test/heap_co*.fits'     # heap_aia_xrt input
...
for i in faia :
    ...
    # predicted=predicted-1-np.log10((xrtpix/aiapix)**2.)   
    predict=10**predicted-1  #de-log10

    # fig=plt.figure(num=netfile,figsize=(8,7))
    # plt.subplot(211)

    # plt.imshow(predicted,cmap=cmap[6], vmin=-3,vmax=5,origin='lower')
    # plt.colorbar()     
    # plt.pause(0.1)
    # plt.draw()
    # if n !=1 and j !=n :
